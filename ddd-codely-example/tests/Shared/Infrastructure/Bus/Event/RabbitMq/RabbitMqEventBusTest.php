...
use CodelyTv\Shared\Infrastructure\Bus\Event\RabbitMq\RabbitMqEventBusInterface;
...
final class RabbitMqEventBusTest extends InfrastructureTestCase
{
	...
	private RabbitMqEventBusInterface $publisher;
	...
	protected function setUp(): void
	{
		...
		$this->publisher = new RabbitMqEventBusInterface(
			$this->connection,
			$this->exchangeName,
			$this->service(MySqlDoctrineEventBusInterface::class)
		);
	...
	private function assertConsumer(AbstractDomainEvent ...$expectedDomainEvents): callable
	{
		return function (AbstractDomainEvent $domainEvent) use ($expectedDomainEvents): void {
	...
	{
		return static function (AbstractDomainEvent $domainEvent): never {
