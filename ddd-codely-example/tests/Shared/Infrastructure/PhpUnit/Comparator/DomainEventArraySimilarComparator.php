...
final class DomainEventArraySimilarComparator extends Comparator
{
	public function accepts($expected, $actual): bool
	{
		return is_array($expected)
			   && is_array($actual)
			   && (all(instance_of(AbstractDomainEvent::class), $expected)
				   && all(instance_of(AbstractDomainEvent::class), $actual));
	}
	...
	{
		$exists = static fn (AbstractDomainEvent $expected): bool => any(
			static fn (AbstractDomainEvent $actual): bool => TestUtils::isSimilar($expected, $actual),
			$actualArray
		);
