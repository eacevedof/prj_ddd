...
final class DomainEventSubscriberLocator
{
	...
	public function withRabbitMqQueueNamed(string $queueName): callable | DomainEventSubscriberInterface
	{
		$subscriber = search(
			static fn (DomainEventSubscriberInterface $subscriber): bool => RabbitMqQueueNameFormatter::format($subscriber) ===
															$queueName,
			$this->mapping
		);
